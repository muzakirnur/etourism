    ...
    <div class="w-full md:w-3/4 mx-auto">
        @php
            $userHadReview = Auth::user()
                ->hotelRating->where('hotel_id', $hotel->id)
                ->first();
        @endphp
        @if ($userHadReview)
    ...
    @if (count($hotel->rating) > 0)
        <div class="w-full lg:w-3/4 mx-auto mb-4">
            <div class="flex items-center justify-center">
                @for ($i = 1; $i < 6; $i++)
                    <svg class="w-4 h-4 {{ $i <= round($hotel->totalRating()) ? 'text-yellow-300' : 'text-gray-500 dark:text-gray-400' }} me-1"
                ...
                <p class="ms-1 text-sm font-medium text-gray-500 dark:text-gray-400">{{ count($hotel->rating) }} Review
                </p>
            </div>
