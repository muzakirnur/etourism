<x-guest-layout>
    ...
    <div class="w-full md:w-3/4 flex mx-auto mb-8">
        <livewire:user.wisata.review :wisata="$wisata" />
    </div>
    @if (count($wisata->rating) > 0)
        <div class="w-full lg:w-3/4 mx-auto mb-4">
            <div class="flex items-center justify-center">
                @for ($i = 1; $i < 6; $i++)
                    <svg class="w-4 h-4 {{ $i <= round($wisata->totalRating()) ? 'text-yellow-300' : 'text-gray-500 dark:text-gray-400' }} me-1"
                ...
                <p class="ms-1 text-sm font-medium text-gray-500 dark:text-gray-400">{{ count($wisata->rating) }} Review
                </p>
            </div>
                ...
                <figure class="w-2/3 border p-4 rounded-lg mx-auto">
                    <div class="flex flex-wrap items-center mb-4">
                        <figcaption class="flex items-center space-x-3 rtl:space-x-reverse w-3/4 align-middle">
                            <img class="w-6 h-6 rounded-full" src="{{ $userHadReview->user->profile_photo_url }}"
                                alt="profile picture">
                            <div
                                class="flex items-center divide-x-2 rtl:divide-x-reverse divide-gray-300 dark:divide-gray-700">
                                <cite
                                    class="pe-3 font-medium text-gray-900 dark:text-white">{{ $userHadReview->user->name }}</cite>
                                <cite
                                    class="ps-3 text-sm text-gray-500 dark:text-gray-400">{{ $userHadReview->jenis_kunjungan }}</cite>
                            </div>
                        </figcaption>
                        <div class="flex items-center mb-4 text-yellow-300 w-1/4 justify-end align-top">
                            @for ($i = 1; $i < 6; $i++)
                                <svg class="w-4 h-4 {{ $i <= $userHadReview->bintang ? 'text-yellow-300' : 'text-gray-500 dark:text-gray-400' }} me-1"
                        ...
                        <div class="w-full">
                            <p class="text-sm font-extralight italic">
                                {{ date('d F Y', strtotime($userHadReview->tanggal)) }}</p>
                        </div>
                    </div>
                    <blockquote>
                        <p class="text-lg font-semibold text-gray-900 dark:text-white">"{{ $userHadReview->ulasan }}"
                        </p>
                    </blockquote>
                    @if ($userHadReview->gambar != null)
                        <div class="p-4">
                            <img src="{{ asset('storage/' . $userHadReview->gambar) }}" alt="Photo"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                async function initMap() {
                    ...
                    const position = {
                        lat: 5.221809197503294,
                        lng: 96.71791035533619
                    };
                        ...
                        if (navigator.geolocation) {
                            ...
                            navigator.geolocation.getCurrentPosition(
                                (position) => {
                                        ...
                                        if (status == 'OK') {
                                            ...
                                            for (var i = 0; i < routes.length; i++) {
                                                ...
                                                for (var j = 0; j < steps.length; j++) {
                                                    stepsCoords[j] = new google.maps.LatLng(
                                    ...
                                    infoWindow.open(map);
                                    map.setCenter(pos);
                                    var url = "https://www.google.com/maps/dir/?api=1&origin=" + pos
                                        .lat + "," + pos.lng + "&destination=" + lokasiWisata.lat +
                                        "," + lokasiWisata.lng;
