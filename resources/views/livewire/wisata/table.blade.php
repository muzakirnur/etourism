        ...
        <tbody class="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
            ...
            @foreach ($data as $row)
                <tr>
                    <td class="p-2">
                        @if ($row->pictures)
                            <img class="w-96 h-48" src="{{ asset('storage/' . $row->pictures->first()?->path) }}"
                                alt="{{ $row->name }}">
                        @endif
                    </td>
                    <td class="p-2">
                    ...
                    <td class="p-2">
                        <div class="w-full text-end">
                            <a href="{{ route('wisata.edit', $row) }}"
                                class="btn bg-indigo-500 hover:bg-indigo-600 text-white whitespace-nowrap">Edit</a>
