...
<div class="relative z-30 inline-flex" x-data="{ open: false }">
    ...
    >
        ...
        <div class="flex items-center truncate">
            <span class="truncate hidden md:block ml-2 text-sm font-medium dark:text-slate-300 group-hover:text-slate-800 dark:group-hover:text-slate-200">{{ Auth::user()->name }}</span>
    ...
    >
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200 dark:border-slate-700">
            ...
            <div class="text-xs text-slate-500 dark:text-slate-400 italic">{{ Auth::user()->is_admin == true ? 'Administrator' : 'User' }}</div>
        </div>
        <ul>
            <li>
